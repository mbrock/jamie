...
func (i *OggWriter) writeHeaders() error {
	...
	binary.LittleEndian.PutUint32(
		oggIDHeader[12:],
		i.sampleRate,
	) // original sample rate, any valid sample e.g 48000
	binary.LittleEndian.PutUint16(
		oggIDHeader[16:],
		0,
	) // output gain
	...
	data := i.createPage(
		oggIDHeader,
		pageHeaderTypeBeginningOfStream,
		0,
		i.pageIndex,
	)
...
func (i *OggWriter) createPage(
	...
	copy(
		page[pageHeaderSize+nSegments:],
		payload,
	) // Payload goes after the segment table, so at pageHeaderSize+nSegments.
...
func (i *OggWriter) Close() error {
	...
	pageOffset, err := i.fd.Seek(
		-1*int64(i.lastPayloadSize+pageHeaderSize+1),
		2,
	)
	if err != nil {
		return err
	...
	fmt.Println("Writing page to stream")
	if err := i.writeToStream(data); err != nil {
		return err
	}
