...
func registerDBQuery(ctx context.Context, queries *db.Queries) {
	pl, err := trealla.New()
	if err != nil {
		log.Fatal("Failed to initialize Prolog", "error", err)
	}
	...
	if err != nil {
		log.Error("Failed to execute Prolog query", "error", err)
	} else {
		log.Info("Last joined channel", "channel", answer.Solution["_"])
	}
}
